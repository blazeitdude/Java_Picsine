...
public class SocketsApplicationConﬁg {
    ...
    @Bean
    public MessageRepository MessageRepositoryBean() {
        return new MessageRepositoryImpl(hikariDataSourceBean());
    }

    @Bean
    UsersService usersServiceBean() {
        return new UsersServiceImpl(UsersRepositoryImplBean(), MessageRepositoryBean(), passwordEncoderBean());
    }
